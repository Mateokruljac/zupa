 ...
 * Liturgijski sadržaj puni PastoralLiturgical (LitCal + HILP).
 */
(function (global) {
  "use strict";
        ...
        return `<div class="nakane-mass-group">
          ...
          ${items
            ...
            <div>
              ...
              <br><small>${esc(n.requestedBy || "—")} · ${num(n.stipend)} €</small>
  ...
  function renderTable() {
    ...
    let filtered = allRows.filter((n) => {
      ...
      if (!q) return true;
      return [n.date, n.massTime, n.intentionFor, n.requestedBy, String(n.stipend)].some((v) =>
          ...
          <tbody>${
            filtered.length
              ? filtered
                  ...
                  .join("")
              : `<tr><td colspan="7" class="empty-state">Nema nakana za prikaz.</td></tr>`
          }</tbody>
        </table>
      </div>`;
  ...
  function nakanePrintRow(n) {
    ...
    return `<tr>
      ...
      <td class="col-requester">${esc(n.requestedBy || "—")}</td>
