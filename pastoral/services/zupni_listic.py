...
from pastoral.services.mass_schedule import format_mass_schedule_html as format_mass_schedule

CONFIG_PATH = Path(__file__).resolve().parent.parent / 'data' / 'zupni_listic_config.json'
...
def render_block_html(block: dict, data: dict, settings: dict, week_start: str) -> str:
    ...
    if btype == 'header':
        return render_to_string('pastoral/listic/header.html', {
            'settings': settings,
            'week_start': start,
            'week_end': end,
            'week_start_fmt': fmt_short(start),
            'week_end_fmt': fmt_short(end),
            'liturgy_color': liturgical_color_hint(start),
        })
    ...
    if btype == 'contact':
        return render_to_string('pastoral/listic/section.html', {
            ...
            'body_html': render_to_string('pastoral/listic/contact.html', {'settings': settings}),
        })
    if btype == 'footer':
        footer_body = plain_text_to_html(body_html) if body_html and '<' not in body_html else body_html
        return render_to_string('pastoral/listic/footer.html', {
...
def render_layout_to_html(layout: dict, data: dict, settings: dict, week_start: str) -> str:
    ...
    return render_to_string('pastoral/listic/document.html', {'inner_html': inner})
...
def sync_auto_blocks(layout: dict, data: dict, settings: dict, week_start: str) -> dict:
    ...
    for b in cfg['defaultLayout']['blocks']:
        if b.get('id') == 'blk_kt':
            default_body = b.get('body') or ''
            break
    for block in layout.get('blocks') or []:
        if block.get('type') == 'custom_text' and block.get('id') == 'blk_kt' and not block.get('body'):
