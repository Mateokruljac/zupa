    ...
    if action_name == 'add_task':
        ...
        if task_form.is_valid():
            parish_data.setdefault('tasks', []).append({
                ...
                'owner': task_form.cleaned_data['owner'],
                'due': (
                    task_form.cleaned_data['due'].isoformat()
                    if task_form.cleaned_data.get('due')
                    else ''
                ),
                'priority': task_form.cleaned_data['priority'],
                'category': task_form.cleaned_data.get('category') or 'ured',
                'done': False,
            })
            parish_data_service.save(parish_data)
    ...
    if action_name == 'add_event':
        ...
        if event_form.is_valid():
            ...
            conflict = next((
                ...
                and event.get('time') == event_time
                and event_time
                and (event.get('place') or '').casefold() == place.casefold()
                and place
            ), None)
            if conflict:
                messages.error(
                ...
                messages.success(
                    request,
                    'Župni događaj dodan u zajednički kalendar.',
                )
        else:
