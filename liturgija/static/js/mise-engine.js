    ...
    M.openForm({
      ...
      body: `
        <div class="nakana-form form-wide">
          <div class="nakana-form__meta">
            <div class="form-group"><label>Misa (sat)</label><input name="massTime" value="${esc(massTime)}" readonly /></div>
            <div class="form-group"><label>Stipendij (€)</label><input name="stipend" type="number" min="0" step="0.01" value="${state.defaultStipend}" /></div>
            <label class="nakana-form__paid"><input type="checkbox" name="paid" /> Stipendij evidentiran</label>
          </div>
          <div class="form-group"><label>Za koga / namjera *</label><input name="intentionFor" required placeholder="Pokoj duše…" /></div>
          <div class="form-group"><label>Bilješka (samo za svećenika)</label><textarea name="notes" rows="2" placeholder="Interna bilješka…"></textarea></div>
        </div>`,
      submitLabel: "Spremi",
      onSubmit: (form) => {
        const fd = new FormData(form);
