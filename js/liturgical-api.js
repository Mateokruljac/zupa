...
(function (global) {
  ...
  async function loadLitcalYearsForMonth(civilYear, month) {
    return loadLitcalYears(calendarYearsForMonth(civilYear, month));
  }
  ...
  function getSummarySync(iso) {
    ...
    if (events?.length) {
      ...
      if (mapped) {
        return {
          ...mapped,
          shortTitle: truncate(mapped.title, 38),
          loaded: true,
        };
      }
    }
  ...
  function renderCalColorLegend() {
    ...
    return `<div class="cal-lit-color-legend" aria-hidden="true">
      ...
      <span class="cal-lit-legend-note">Podaci: LitCal API v5 (ili lokalna kopija u projektu) · latinski nazivi; za hrvatski tekst koristite HILP.</span>
    </div>`;
  }
  ...
  function renderNakaneDayLiturgy(day) {
    ...
    return `<div class="nakane-lit-detail ${bgCls}" id="nakane-lit-detail">
      ...
      <p class="lit-source-note">Izvor: ${esc(sourceNote)} · latinski naziv; za hrvatski tekst koristite HILP.</p>
    </div>`;
  }
