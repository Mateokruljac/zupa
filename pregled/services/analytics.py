...
from financije.ledgers import PARISH_BALANCE_LEDGER, entry_ledger
from financije.services.debts import collect_debts, cat_meta
...
def compute_cashbook_months(data: dict, months: int = 6) -> list[dict]:
    ...
    for i in range(months - 1, -1, -1):
        ...
        entries = [
            entry
            for entry in data.get('cashbook') or []
            if (entry.get('date') or '').startswith(mk)
            and entry_ledger(entry) == PARISH_BALANCE_LEDGER
        ]
